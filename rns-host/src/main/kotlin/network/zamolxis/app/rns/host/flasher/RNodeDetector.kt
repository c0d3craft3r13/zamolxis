...
class RNodeDetector(
     ...
     */
    suspend fun getDeviceInfo(): RNodeDeviceInfo? =
        withContext(Dispatchers.IO) {
            try {
                ...
                val board =
                    romInfo?.product?.let { RNodeBoard.fromProductCode(it) }
     ...
     */
    suspend fun indicateFirmwareUpdate(): Boolean =
        withContext(Dispatchers.IO) {
     ...
     */
    suspend fun getFirmwareHash(): ByteArray? =
        withContext(Dispatchers.IO) {
     ...
     * @return true if the command was sent successfully
     */
    suspend fun setFirmwareHash(hash: ByteArray): Boolean =
        withContext(Dispatchers.IO) {
    ...
    suspend fun provisionDevice(
        ...
        withContext(Dispatchers.IO) {
            Log.i(
                TAG,
                "Provisioning device: product=0x${(product.toInt() and 0xFF).toString(16)}, " +
     ...
     */
    suspend fun disableTncMode(): Boolean =
        withContext(Dispatchers.IO) {
