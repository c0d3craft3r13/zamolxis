...
1. Open Sentry dashboard for Zamolxis project
2. Filter events by:
...
2. Leave app running overnight with screen off
   ...
   - Disable battery optimization for Zamolxis
   - Leave on a stable network (WiFi preferred)

3. Next morning, check:
   - Is app still running? `adb shell ps | grep lxmf`
   - Memory usage: `adb shell dumpsys meminfo network.zamolxis.app | grep -A 5 TOTAL`
