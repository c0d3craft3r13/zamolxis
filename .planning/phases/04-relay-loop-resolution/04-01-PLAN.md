...
<task type="auto">
  ...
  <files>app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt</files>
  <action>
...
- The 30-second cooldown after selection - still valuable for stability
  ...
  <verify>
1. Run: `grep -n "SharingStarted.WhileSubscribed" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt`
   Expected: 3 matches (lines ~202, ~212, ~236)

2. Run: `grep -n "SharingStarted.Eagerly" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt`
...
The existing tests use mocked StateFlows in the test setup, so they don't directly test the SharingStarted strategy. However, they verify the core state machine logic remains intact.
  ...
  <verify>
1. Run: `cd /home/tyler/repos/public/zamolxis-issue-342-map-location-dialog && JAVA_HOME=/home/tyler/android-studio/jbr ./gradlew :app:compileDebugKotlin`
   Expected: BUILD SUCCESSFUL

2. Run: `cd /home/tyler/repos/public/zamolxis-issue-342-map-location-dialog && JAVA_HOME=/home/tyler/android-studio/jbr ./gradlew :app:testDebugUnitTest --tests "network.zamolxis.app.service.PropagationNodeManagerTest"`
...
DO NOT modify this code - just verify it's present.
  ...
  <verify>
Run: `grep -n "Relay selection loop detected" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt`
Expected: 1 match showing Sentry.captureMessage call (line ~841)

Run: `grep -n "recordSelection" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt`
...
1. **Code verification:**
   - `grep -c "WhileSubscribed(5000L)" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt` returns 3
   - `grep -c "SharingStarted.Eagerly" app/src/main/java/network.zamolxis.app/service/PropagationNodeManager.kt` returns 0
