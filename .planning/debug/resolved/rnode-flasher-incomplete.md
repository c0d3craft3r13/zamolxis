---
status: fixing
trigger: "Investigate why Zamolxis's RNode flasher fails to restart the device and leaves it showing 'Missing Config', while rnodeconf's --autoinstall works correctly."
...
test: Compare how rnodeconf calculates partition hash vs how Zamolxis calculates firmware binary hash
expecting: The hash methods are different - rnodeconf extracts from embedded hash in .bin, Zamolxis calculates full SHA256
next_action: Verify the hash calculation difference and ensure Zamolxis uses the correct method

## Symptoms

expected: Device restarts after flashing and shows proper status (not "Missing Config")
actual: Device doesn't auto-restart and shows "Missing Config" after Zamolxis flashing
errors: None - just wrong behavior
reproduction: Flash a Heltec LoRa32 v4 with Zamolxis's flasher vs rnodeconf --autoinstall
started: Discovered during manual testing of PR #301

## Eliminated

- hypothesis: Missing EEPROM provisioning
  evidence: Zamolxis's RNodeDetector.provisionDevice() writes all required fields (product, model, hwrev, serial, timestamp, checksum, signature, lock byte)
  timestamp: 2026-01-28T12:10:00Z

- hypothesis: "Missing Config" means device is broken
  evidence: "Missing Config" is the NORMAL display for devices in "host-controlled mode" used by Reticulum apps (Zamolxis, Sideband, MeshChat). TNC mode (saved radio config) is NOT needed.
...
- timestamp: 2026-01-28T12:05:00Z
  ...
  implication: Zamolxis has all these steps implemented but may have a hash calculation mismatch
...
- timestamp: 2026-01-28T12:08:00Z
  ...
  found: get_partition_hash() for ESP32 does NOT calculate SHA256 of entire binary. Instead:
    ...
    ```
  implication: ESP32 firmware has SHA256 hash embedded in last 32 bytes! rnodeconf extracts this, Zamolxis calculates from scratch.

- timestamp: 2026-01-28T12:10:00Z
  checked: Zamolxis FirmwarePackage.calculateFirmwareBinaryHash()
...
- timestamp: 2026-01-28T12:12:00Z
  checked: Zamolxis RNodeDetector.provisionAndSetFirmwareHash()
...
- timestamp: 2026-01-28T12:14:00Z
  ...
  found: rnodeconf uses `--after hard_reset` flag which triggers automatic reset after flashing. Zamolxis's ESPToolFlasher does send FLASH_END with reboot=true and calls hardReset(), but for ESP32-S3 native USB devices the reset may not work reliably.
  implication: For ESP32-S3 devices, Zamolxis correctly detects this and shows FlashState.NeedsManualReset

## Resolution

root_cause: TWO ISSUES FOUND:
1. Firmware hash calculation was incorrect for ESP32 devices. The RNode firmware embeds the SHA256 hash in the last 32 bytes of the application binary. rnodeconf extracts this embedded hash, while Zamolxis was calculating SHA256 of the ENTIRE binary (including the embedded hash), producing a wrong value.
...
files_changed:
- reticulum/src/main/java/network.zamolxis.app/reticulum/flasher/FirmwarePackage.kt
- reticulum/src/main/java/network.zamolxis.app/reticulum/flasher/RNodeFlasher.kt
