...
internal object RnsConfigFile {
     ...
     *
     * Two independent shared-instance modes drive the emission:
     *
     * - `joinShareInstance = true` → "we're an RPC client of a co-located
     *   master." Emits `share_instance = yes` and skips the `[interfaces]`
     ...
     *   `share_instance = yes` AND keeps the `[interfaces]` block (we own
     *   the transport). No `rpc_key` — upstream RNS generates one for us
     *   at runtime. Driven by the user's Advanced-settings toggle plumbed
     *   through `ReticulumConfig.shareInstanceHosting`.
     *
     * The two modes are mutually exclusive in practice: when both are
     * computed true, the caller resolves `host` to false because another
     * master already holds TCP 37428 (kernel-level constraint — upstream's
     * first-to-bind policy). See `PythonRnsRuntime.start()` for the
     * resolution + UI-visible "conflict" mode.
     *
     * `skipAutoInterface = true` force-disables AutoInterface entries
     * (with a `# WARNING: ...` comment) — the caller has determined the
     * multicast bind would conflict with another RNS app on the device.
     */
    fun build(
        config: ReticulumConfig,
        joinShareInstance: Boolean = false,
        hostShareInstance: Boolean = false,
        skipAutoInterface: Boolean = false,
    ): String {
        val shareInstance = joinShareInstance || hostShareInstance
        ...
        // Instance Access). Only meaningful when we're the client of a
        // co-located master — gate on `joinShareInstance` (the host branch
        // is the master and upstream RNS generates its own RPC key).
        if (joinShareInstance) {
        ...
        // Skip the `[interfaces]` block only when joining a master — the
        // host branch owns the transport and still needs its interface list.
        if (!joinShareInstance) {
            sb.appendLine()
