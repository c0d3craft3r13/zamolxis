...
object LinkCostResolver {
     ...
     * cannot drift when an interface is renamed, and the `when` is exhaustive
     * with no `else`, so adding a variant to [InterfaceType] fails to compile
     * until someone has decided what it costs. An `else` branch here would have
     * silently classified every future radio interface as cheap — which is the
     * direction that spends other people's airtime.
     */
    fun costOf(interfaceType: InterfaceType): LinkCost =
        when (interfaceType) {
            ...
            -> LinkCost.CHEAP
            // An unclassified interface is not assumed cheap. It is far more often
            // IP than LoRa, but the string overload has to make the same guess from
            // less information, and being consistent between the two matters more
            // than the guess itself.
            InterfaceType.UNKNOWN -> LinkCost.CHEAP
        }
     ...
     * Wi-Fi. Nothing recorded yet is also cheap — an unknown path is far more
     * often IP than LoRa, and being wrong here costs airtime rather than secrecy.
     */
