...
/**
 * Encrypts as it is written, so the plaintext never has to exist as a whole.
 *
 * The producer — a ZipOutputStream, in practice — writes straight through this
 * into the file. That is what removes the unencrypted archive v2 left on disk
 * before overwriting it in place, which on flash is not erasure at all.
 *
 * One full chunk is always held back, because a chunk has to be told whether it
 * is the last one before it can be sealed, and that is only known when the next
 * write arrives or the stream closes.
 */
internal class ChunkEncryptingStream(
    private val sink: DataOutputStream,
    dataKey: ByteArray,
    private val noncePrefix: ByteArray,
) : OutputStream() {
    private val key = SecretKeySpec(dataKey, "AES")
    private val buffer = ByteArray(MigrationContainer.CHUNK_SIZE)
    ...
    override fun write(
        ...
        while (remaining > 0) {
            val taken = minOf(remaining, buffer.size - filled)
            source.copyInto(buffer, filled, offset, offset + taken)
            filled += taken
            offset += taken
            remaining -= taken
            if (filled == buffer.size) rotate()
        }
    }

    /** The buffer is full, so whatever was held back cannot have been the last. */
    private fun rotate() {
        held?.let { emit(it, isLast = false) }
        held = buffer.copyOf()
        filled = 0
    }
    ...
    override fun close() {
        ...
        held?.let { emit(it, isLast = false) }
        held = null
        // Always a final chunk, even when empty: a payload that divides evenly
        // into CHUNK_SIZE still needs something to carry the end marker.
        emit(buffer.copyOf(filled), isLast = true)
        sink.flush()
    }
    ...
    private fun emit(
        ...
        cipher.init(Cipher.ENCRYPT_MODE, key, GCMParameterSpec(GCM_TAG_BITS, chunkNonce(noncePrefix, index)))
