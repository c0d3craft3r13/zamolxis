...
internal class BleGattServer(
    ...
    /**
     * Whether the Reticulum service is registered and discoverable.
     *
     * Android starts accepting GATT connections the moment [open] calls
     * `openGattServer`, several milliseconds before `addService` completes. A
     * peer that already knows this device's address reconnects straight into
     * that window — observed on a Motorola Edge 60 Pro:
     *
     * ```
     * 13:32:56.642  registerCallback()
     * 13:32:56.645  onServerConnectionState connected=true   <- central is in
     * 13:32:56.646  open(): adding service...
     * 13:32:56.650  onServiceAdded() status=0                <- only now usable
     * 13:33:26.659  Identity handshake timeout               <- 30 s wasted
     * ```
     *
     * Such a central is attached to a server with no identity characteristic to
     * write, so its handshake can never complete; announcing it upstream only
     * buys a 30-second timeout and a 60-second blacklist. Marked volatile
     * because it is written from the `open`/`close` coroutine and read from the
    ...
    /**
     * Centrals that connected while [serviceReady] was still false, held until
     * the service exists so they can be announced instead of thrown away.
     */
    private val earlyCentrals = java.util.concurrent.ConcurrentHashMap.newKeySet<String>()
    ...
    suspend fun open(): Result<Unit> = withContext(Dispatchers.Main) {
        try {
            ...
            // Only now can a central do anything useful here — see [serviceReady].
            serviceReady = true
            releaseEarlyCentrals()
    ...
    private suspend fun handleConnectionStateChange(
        ...
        when (newState) {
            BluetoothProfile.STATE_CONNECTED -> {
                ...
                if (!serviceReady) {
                    // Connected before the Reticulum service finished registering
                    // (see [serviceReady]). Hold it: announcing now would start a
                    // handshake against characteristics that do not exist yet, and
                    // dropping it would throw away the connection entirely — a
                    // central that reached us through connectGatt(autoConnect) has
                    // spent its parked request and may not re-arm for minutes.
                    // Registration finishes microseconds later; [releaseEarlyCentrals]
                    // announces it then.
                    Log.d(TAG, "Central connected before service registration: $address, holding")
                    earlyCentrals.add(address)
                    return
                }
    ...
    /**
     * Announce the centrals held by [handleConnectionStateChange] while the
     * service was registering. Only those still connected are released — one
     * that gave up in the meantime has already been cleaned out of
     * [connectedCentrals] by its disconnect callback.
     */
    private suspend fun releaseEarlyCentrals() {
        val held = earlyCentrals.toList()
        earlyCentrals.clear()
        for (address in held) {
            val stillConnected = stateMutex.withLock { connectedCentrals.containsKey(address) }
            if (stillConnected) {
                Log.d(TAG, "Releasing held central $address now that the service is registered")
                _centralConnected.tryEmit(address)
            }
        }
    }
