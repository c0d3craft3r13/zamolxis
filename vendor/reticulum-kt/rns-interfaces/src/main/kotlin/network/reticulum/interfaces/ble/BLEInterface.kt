...
class BLEInterface(
    ...
    /**
     * The coroutine running each in-flight incoming handshake, by address.
     *
     * Held so [collectDisconnections] can cancel one when its link goes away.
     * Without that the handshake sits out the full
     * [BLEConstants.IDENTITY_HANDSHAKE_TIMEOUT_MS] on a connection that died
     * seconds earlier, and then blacklists the peer for a minute for a failure
     * that was the radio's, not the peer's. On a link that needs a couple of
     * attempts to settle that alone prevents the mesh from ever forming.
     */
    private val incomingHandshakeJobs = ConcurrentHashMap<String, Job>()
    ...
    private suspend fun collectIncomingConnections() {
        driver.incomingConnections.collect { connection ->
            ...
            // Started lazily so the job is registered before it can finish and
            // try to remove itself — otherwise a fast handshake would leave a
            // stale entry that later cancels an unrelated connection.
            val handshakeJob =
                scope.launch(start = CoroutineStart.LAZY) {
                    try {
                        ...
                        incomingHandshakeJobs.remove(address)
                    }
                }
            incomingHandshakeJobs[address] = handshakeJob
            handshakeJob.start()
        }
    }
    ...
    private suspend fun collectDisconnections() {
        driver.connectionLost.collect { address ->
            // The link is gone, so any handshake still waiting on it is waiting
            // for nothing. Cancelling releases the connection slot now and skips
            // the blacklist the timeout path would otherwise apply — see
            // [incomingHandshakeJobs].
            incomingHandshakeJobs.remove(address)?.let { job ->
                if (job.isActive) {
                    log("Link lost during handshake with ${address.takeLast(8)}, cancelling it")
                    job.cancel()
                }
            }
