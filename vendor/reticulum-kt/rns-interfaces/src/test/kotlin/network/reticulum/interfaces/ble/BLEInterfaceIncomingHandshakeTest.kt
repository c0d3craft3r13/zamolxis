...
class BLEInterfaceIncomingHandshakeTest {
    ...
    fun `two rapid incoming connections for same MAC start only one handshake`() {
        runBlocking {
            ...
            assertTrue(driver.disconnectCalls.isEmpty(), "expected no disconnect calls")
        }
    }
    ...
    fun `incoming connection from blacklisted MAC is rejected without starting handshake`() {
        runBlocking {
            ...
            // a 30s timeout.
            assertEquals(0, conn.receivedFragmentsSubscribers)
    ...
    @Test
    fun `a lost link cancels its in-flight handshake instead of waiting out the timeout`() {
        runBlocking {
            val driver = StubBLEDriver()
            val iface = newInterface(driver)
            ...
            val conn = FakeBLEPeerConnection(address = DUP_MAC)
            driver.incomingConnections.emit(conn)
            waitUntil("handshake subscribes to receivedFragments") {
                conn.receivedFragmentsSubscribers >= 1
            }

            // The GATT link drops moments after connecting — the common case on
            // a marginal radio. Before this was handled, the handshake sat out
            // the full IDENTITY_HANDSHAKE_TIMEOUT_MS (30 s) on a dead
            // connection and then blacklisted the peer for a minute, so a link
            // that needed a couple of attempts could never establish.
            withTimeout(2_000) {
                while (driver.connectionLost.subscriptionCount.value < 1) {
                    delay(10)
                }
            }
            driver.connectionLost.emit(DUP_MAC)

            waitUntil("in-flight handshake is released") {
                conn.receivedFragmentsSubscribers == 0
            }

            // Cancelled, not failed: the peer must stay connectable so the very
            // next attempt can succeed.
            assertNull(iface.getBlacklistEntryForTest(DUP_MAC))
        }
    }
    ...
    fun `concurrent addToBlacklist calls increment failureCount exactly once per call`() {
        ...
        assertEquals(concurrentCallers, entry.failureCount)
        ...
        assertTrue(
            entry.failureCount <= concurrentCallers,
            "failureCount ${entry.failureCount} exceeded the $concurrentCallers callers",
        )
    }
    ...
    private class StubBLEDriver : BLEDriver {
        ...
        override val connectionLost = MutableSharedFlow<String>(replay = 0)
