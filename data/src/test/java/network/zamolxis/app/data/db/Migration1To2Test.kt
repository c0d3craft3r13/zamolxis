 ...
 * `ZamolxisDatabase.MIGRATION_1_2` uses to lift reactions out of the
 * legacy `fields[16].reactions` overload into the dedicated
 * `reactionsJson` column.
 *
 * Runs without Robolectric / Room — exercises [ZamolxisDatabase.splitReactionsOutOfFieldsJson]
...
class Migration1To2Test {
    ...
    fun `splits reactions blob and leaves remaining field16 keys in place`() {
        ...
        val split = ZamolxisDatabase.splitReactionsOutOfFieldsJson(input)
    ...
    fun `removes field16 entirely when reactions was its only key`() {
        ...
        val (newFieldsJson, _) = requireNotNull(ZamolxisDatabase.splitReactionsOutOfFieldsJson(input))
    ...
    fun `returns null when there are no reactions to lift`() {
        ...
        assertNull(ZamolxisDatabase.splitReactionsOutOfFieldsJson(withoutReactions))
    }

    @Test
    fun `returns null when field16 is missing`() {
        val noField16 = """{"1": "text", "6": "image_hex"}"""
        assertNull(ZamolxisDatabase.splitReactionsOutOfFieldsJson(noField16))
    }

    @Test
    fun `returns null on unparseable input`() {
        assertNull(ZamolxisDatabase.splitReactionsOutOfFieldsJson("not valid json {"))
    }
    ...
    fun `multiple emojis survive the round-trip`() {
        ...
        val (newFieldsJson, reactionsJson) =
            requireNotNull(ZamolxisDatabase.splitReactionsOutOfFieldsJson(input))
